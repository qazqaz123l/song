...
/*
    1.首先创建函数对象,需要输入参数应该有行和列，还有雷的数量
    2.创建表格，利用每个函数都有的属性原型prototype，继承原函数
    3.创建初始化函数方便调用
    4.创建随机出现雷的函数
    5.创建数组，存放方块的信息，看存放的是雷还是数字，坐标跟行列正好相反，取得时候用行列，但找数据的时候用坐标方便
    6.创建玩游戏的函数，用来实现鼠标点击事件玩游戏
    7.玩游戏函数中点击的是数字要进行判断是0还是其他数，是0就不显示数字，并搜索周围的是0就出现一片空白，利用递归
    8.因此需要创建个搜索周围方块的函数
    9.点击的不是数字是雷游戏结束函数
    10.更新数字值函数
    11.右击出现小红旗,首先判断有没有标记过类名，并且是不是小红旗,在此之前首先把右键出现菜单的操作在初始化时去掉
    12.判断小红旗后面是不是雷，都是雷，给变量赋值true
    13.计算剩余雷数量，既是点击次数，有几个雷红旗就只能标几次
    14.判断是否成功，首先就是判断点击次数是否为0，在判断变量是否都为true
    15.最后就是级别按钮选择
    16.最后别忘了去掉下一次点击级别父级的内容，否则三种级别的表格都会显示在网页上
